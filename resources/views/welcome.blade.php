...
<div class="row">
    <div class="col-md-6 col-xl-12">
        <div class="card stat-widget">
            <div class="card-body">
                <h5 class="card-title">Hi {{ auth()->user()->name }} - <small>{{ auth()->user()->company->name }} ({{ auth()->user()->company->ssm }})</small></h5>
                @if (count($announcements) == 0)
                <p class="card-text">No announcement.
                    @if (auth()->user()->level == 1)
                    <a href="{{ route('dashboard.announcement.create') }}">Create Now</a>
                    @endif
                </p>
                @endif
